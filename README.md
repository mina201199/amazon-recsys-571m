...
你在 Amazon 買過東西之後，首頁會出現「為你推薦」。這個專案就是**從零做出那個功能**。

難的地方是 Amazon 上有**四千八百萬種商品**。要從裡面挑 10 個推薦給你，
又不能真的一個一個去比對——那樣你打開網頁要等好幾分鐘。

解法是**分兩關**，就像徵才：一個職缺收到四千八百萬封履歷，不可能每個人都面談三小時，
所以先用幾個條件快速篩到剩幾百份，再對這幾百份仔細看。
推薦系統一模一樣，只是「快」和「慢」指的是電腦的計算時間。

這個 repo 完整實作了第一關（從四千八百萬篩到五百），第二關（從五百挑十個）
已有設計但尚未接上。
...
輸出一份排序過的 Top-10 推薦清單。這個問題的難處不在演算法，而在**規模**：
...
驗證段（2023-03 ~ 2023-06）、21,317 位使用者、候選數 K=500。
特徵只使用 2023-03-01 之前的資料，由程式強制。

| 召回通道 | Recall@10 | Recall@500 | NDCG@10 | 覆蓋率 |
|---|---|---|---|---|
| 熱門商品（基線） | 0.0023 | 0.0252 | 0.0015 | 0.0000 |
| 共現統計 | 0.0027 | 0.0062 | 0.0020 | 0.0012 |
| ALS 矩陣分解 | 0.0012 | 0.0104 | 0.0011 | 0.0001 |
| **合併兩路**（熱門＋共現） | **0.0037** | **0.0292** | **0.0023** | 0.0012 |
| 合併三路（再加 ALS） | 0.0030 ↓ | 0.0243 ↓ | 0.0021 | 0.0012 |

合併兩路勝過任何單一通道——多路召回的價值在這裡成立。
但**再加入第三路反而變差**，原因見下。

絕對值低，但參照點是**從 4819 萬個商品中猜中使用者接下來會買的那 2.26 個**。
隨機猜的期望值在 0.00000005 量級，熱門基線已是隨機的四萬倍。
脫離基線談絕對值沒有意義，這也是報告一定要有基線階梯的原因。

#### 兩個獨立實驗，同一個結論：瓶頸在合併層

**證據一 — 把既有通道放寬。** 修掉共現的效能瓶頸後 fit 從十幾分鐘降到 16 秒，
於是放寬參數、預期召回提升。結果相反：

| | 收緊參數 | 放寬參數 |
|---|---|---|
| 鄰居圖邊數 | 455,842 | 18,538,480 |
| 有鄰居的商品 | 116,485 | 1,727,364 |
| 共現自己的 Recall@500 | 0.0062 | 0.0126 ↑ |
| **合併後 Recall@10** | **0.0037** | **0.0028 ↓** |
| fit 耗時 | 16.3 s | 239.7 s |

**證據二 — 加入全新通道。** ALS 訓練了 1250 萬使用者、1152 萬商品（18.6 分鐘），
但它自己的 Recall@10 是三路最低（0.0012，比最簡單的熱門商品還差），
加進合併後把整體從 0.0037 拉低到 0.0030。

**兩次都讓結果變差，病因完全相同**：合併採輪流交錯、各路權重相等，
所以任何一路變弱或變吵都會直接稀釋最終名單。

兩條獨立路徑指向同一個瓶頸，代表**限制真的在合併層，不在任何一路的參數**。
下一步該做的是讓合併依通道品質加權，而不是繼續調參或加通道。

#### 一個 53 分鐘的實作錯誤

ALS 產生推薦花了 53 分鐘，比訓練還久。原因是逐一使用者比對全部 1152 萬件商品
——兩萬位使用者就是 7.8 兆次運算，且每次重新走訪整份因子矩陣。
改成疊成矩陣一次相乘後交給 BLAS 處理。

**這不是演算法問題，是沒有善用工具。** 會發現它，是因為評估腳本把各步驟的
耗時分開記錄——否則只會以為「ALS 本來就慢」。

### 開發狀態

已完成：資料管線、召回層（熱門、共現、ALS）、評估框架、75+ 個測試、
全量轉檔與品質報告。

未完成：LightGBM 排序層已有設計但尚未接上；合併層的加權策略（上述兩個實驗
指出的真正瓶頸）待改。這兩項是明確的下一步。
