...
| **合併兩路** | **0.0037** | **0.0292** | **0.0023** | 0.0012 |

合併後勝過任何單一通道——多路召回的價值在這裡被證實。
...
修掉共現通道的效能瓶頸後，fit 從十幾分鐘降到 16 秒，於是放寬參數、預期召回提升。
結果相反：
...
合併採輪流交錯、兩路權重相等，共現放寬後產生的大量弱候選在名單前段擠掉了
熱門商品的候選。**限制不在圖的大小，在合併策略**——下一步該改合併層，
而不是繼續調共現參數。
...
未完成：ALS 尚未在全量資料上執行（5451 萬使用者的 Cholesky 求解需另外規劃）；
LightGBM 排序層已有設計但尚未接上。這兩項是明確的下一步。
